...
class CheapGate:
    ...
    def __init__(self, engine: 'CrystalEngine', file_io: FileIO):
        ...
        self._search_threshold = 3  # 连续3轮未搜索则触发提醒
    ...
    def check(self, user_input: str, history: List[Tuple[str, str]]) -> Dict[str, Any]:
        ...
        Returns:
            {
                "pass": bool,               # 是否通过便宜门
                "action": str,              # bypass / call_llm / trigger_reminder
                "matched_rule": str,        # 命中的规则名
                "reason": str,              # 决策原因
                "skip_llm": bool            # 是否可以跳过LLM
            }
        """
        results = []

        # ---- 规则1：明确指令检测 ----
        instruction_result = self._check_instructions(user_input)
        results.append(instruction_result)
        if instruction_result.get("direct_match"):
            return {
                "pass": True,
                "action": "bypass",
                "matched_rule": "instruction_detected",
                "reason": f"匹配明确指令: {instruction_result.get('instruction_type')}",
                "skip_llm": True
            }

        # ---- 规则2：简单问答检测 ----
        simple_result = self._check_simple_question(user_input)
        results.append(simple_result)
        if simple_result.get("is_simple"):
            return {
                "pass": True,
                "action": "bypass",
                "matched_rule": "simple_question",
                "reason": "简单问题，无需LLM",
                "skip_llm": True
            }

        # ---- 规则3：连续3轮未搜索检测 ----
        search_result = self._check_search_frequency(history)
        results.append(search_result)
        if search_result.get("need_reminder"):
            return {
                "pass": True,
                "action": "trigger_reminder",
                "matched_rule": "search_frequency_decay",
                "reason": "连续3轮未搜索，建议补充外部信息",
                "skip_llm": False,
                "reminder": "[WARN] 连续3轮对话未进行外部搜索，建议先搜索再分析。"
            }

        # ---- 规则4：纯情绪/确认检测 ----
        emotion_result = self._check_emotion_only(user_input)
        results.append(emotion_result)
        if emotion_result.get("is_emotion_only"):
            return {
                "pass": True,
                "action": "bypass",
                "matched_rule": "emotion_only",
                "reason": "纯情绪表达或简单确认，无需LLM",
                "skip_llm": True
            }

        # ---- 规则5：成本估算 ----
        cost_estimate = self._estimate_cost(user_input, history)
        ...
        return {
            ...
            "reason": f"未命中便宜门规则，估算Token消耗: {cost_estimate.get('estimated_tokens', 100)}",
            "skip_llm": False,
...
class AIClient:
    ...
    def _call_api(self, messages: List[Dict], temperature: float = 0.7,
        ...
        # 在 _call_api 方法开头添加统计
        if not hasattr(self, '_call_count'):
            self._call_count = 0
            self._token_estimate = 0
        self._call_count += 1
        # 粗略估算 token（中文约2字符/token）
        total_chars = sum(len(m.get("content", "")) for m in messages)
        self._token_estimate += total_chars // 2
        if not self.api_key:
...
class DebateEngine:
    ...
    def __init__(self, ai: AIClient, engine: CrystalEngine, roles: List[Dict], log: LogFn = None, stream_callback: Callable = None, progress_callback: Callable = None):
        ...
        self.roles = [
            ...
            for idx, item in enumerate(roles[:6])
        ]
...
要求：保留每个视角的最强洞察，删除重复内容，输出不超过 400 字。"""
    ...
    def _core_roles(self) -> List[DebateRole]:
        priorities = ("radical", "conservative", "structural")
        by_key = {role.key: role for role in self.roles}
        selected = [by_key[key] for key in priorities if key in by_key]
        if len(selected) < 3:
            selected_keys = {role.key for role in selected}
            selected.extend([role for role in self.roles if role.key not in selected_keys][:3 - len(selected)])
        return selected[:3]
...
"""
    ...
    def run(self, question: str, mode: str = "debate_full", max_rounds: int = 4) -> Dict:
        ...
        all_roles = core_roles + [r for r in self.roles if r.key == "lark"]
        ...
        round1_answers = self._independent_round(
            ...
            baseline_answer=lark_bare_answer  # 新增参数
        )
    ...
    # ---- 修改开始：_spokesperson 重构，移除截断，强制输出完整字数 ----
...
def _load_roles() -> List[Dict[str, str]]:
    ...
    for key, item in raw.items():
        if isinstance(item, dict):
            roles.append({"key": key, "name": item.get("name", key), "instruction": item.get("instruction", "")})
    fallback_roles = [
    ...
    existing_keys = {item.get("key") for item in roles}
