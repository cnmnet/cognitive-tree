...
class DebateEngine:
    ...
    def _call_role_with_retry(self, role: DebateRole, prompt: str, system: str, max_retries: int = 2) -> str:
        """
        带重试机制的安全AI调用，为每个线程创建独立的 AIClient。
        - 避免共享 Session 导致的连接池排队
        - 不降级为占位文本，确保每个角色的完整输出
        """
        # 每个线程使用独立的 AIClient，避免共享 Session
        ai_client = AIClient(api_key=Config.get_api_key())
        
        for attempt in range(max_retries + 1):
            try:
                return ai_client.chat(prompt, system=system)
            except Exception as e:
    ...
    def _call_role_with_retry(self, role: DebateRole, prompt: str, system: str, max_retries: int = 2) -> str:
        """
        带重试机制的安全AI调用。
        注意：此方法会为每次调用创建独立的 AIClient，确保线程安全。
        """
        # 创建独立的 AI 客户端，确保线程安全
        ai_client = AIClient(api_key=Config.get_api_key())
...
"""
        ...
        answer = self._call_role_with_retry(lark_role, prompt, system)
...
"""
    ...
    def _independent_round(self, question: str, crystal_context: str, roles: List[DebateRole], baseline_answer: str = "") -> List[Dict]:
        ...
        for role in roles:
            ...
            role_tasks.append((role, prompt, system))

        # 定义独立的执行函数
        def execute_role_task(role, prompt, system):
            """执行单个角色任务，使用独立的 AIClient"""
            ai_client = AIClient(api_key=Config.get_api_key())
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=len(role_tasks)) as executor:
            future_to_role = {}
            for role, prompt, system in role_tasks:
                future = executor.submit(execute_role_task, role, prompt, system)
    ...
    # ---- 新增：并发执行 Round 0（百灵鸟裸模型）+ Round 1（核心角色）----
    def _parallel_round0_and_round1(self, question: str, crystal_context: str, core_roles: List[DebateRole]) -> Tuple[List[Dict], str]:
        """
        并发执行：
        - Round 0：百灵鸟裸模型基线（1个任务）
        - Round 1：核心角色独立发言（N个任务）
        
        返回：(round1_answers, lark_bare_answer)
        """
        import time
        import concurrent.futures
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=total_tasks) as executor:
            ...
            # 提交 Round 1 角色任务
            for idx, (role, prompt, system) in enumerate(role_tasks):
                key = f"role_{idx}"
                future = executor.submit(execute_role_task, role, prompt, system)
        ...
        # 按原始顺序组装 Round 1 答案
        for idx, (role, _, _) in enumerate(role_tasks):
            key = f"role_{idx}"
            if key in results:
    ...
    def _debate_round(self, question, crystal_context, previous, audit, round_no, roles, lark_answer=None):
        """后续辩论轮：并发执行所有角色（每个线程独立 AIClient）"""
        import time
        import concurrent.futures
...
"""
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=len(role_tasks)) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_role):
                ...
                try:
                    result = future.result(timeout=180)
                    self.log(f"✅ {role.name} 第 {round_no} 轮发言完成", "system")
                    answers.append({"role": role.name, "answer": result})
                except concurrent.futures.TimeoutError:
    ...
    def _reflection_round(self, question, crystal_context, previous_answers, audit, roles, round_num):
        ...
        # 定义独立的执行函数
        def execute_reflection_task(role):
            """执行单个角色的反思任务，使用独立的 AIClient"""
            ai_client = AIClient(api_key=Config.get_api_key())
            feedback = (audit.get("feedback_by_role") or {}).get(role.name, "")
            prompt = f"""用户问题：{question}
...
"""
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=len(roles)) as executor:
            future_to_role = {}
            for role in roles:
                future = executor.submit(execute_reflection_task, role)
                future_to_role[future] = role
            ...
            for future in concurrent.futures.as_completed(future_to_role):
                ...
                try:
                    result = future.result(timeout=180)
                    self.log(f"✅ {role.name} 反思完成", "system")
                    answers.append({"role": role.name, "answer": result})
                except concurrent.futures.TimeoutError:
...
"""
    ...
    def run(self, question: str, mode: str = "debate_full", max_rounds: int = 4) -> Dict:
        ...
        try:
            ...
            self.log(f"⚠️ 认知风格加载失败，使用默认：{self._cognitive_operators}", "warning")
...
class CrystalTreeApp:
    ...
    def _create_center_panel(self):
        ...
        # ===== 按新顺序创建：报告在上，日志在下 =====
        # 报告面板：expand=True 让它可以扩展，fill=tk.BOTH 填充空间
        self._create_result_panel(center_frame)
        self.result_panel_frame.pack(fill=tk.BOTH, expand=True, padx=12, pady=6)
    ...
    def _create_log_card(self, parent):
        ...
        self.log_area = scrolledtext.ScrolledText(
            self.log_panel_frame,  # 改为使用 self.log_panel_frame
            wrap=tk.WORD,
            height=14,
    ...
    def _create_result_panel(self, parent):
        ...
        self.result_area = scrolledtext.ScrolledText(
            self.result_panel_frame,  # 改为使用 self.result_panel_frame
            wrap=tk.WORD,
            height=24,
            font=("微软雅黑", 14),
            fg=Config.GUI_FG_TEXT,
    ...
    def task():
        ...
        if effective_mode in ("debate_light", "debate_full", "lushi_sampling"):
            ...
            try:
                ...
                result["judge_audit"] = final_schema.judge_audit
                ...
                result["final"] = {
                    "rigid_core": {
                        ...
                        "core_adoptions": [f"{r.get('target_role', '')}：{r.get('verdict', '')}" for r in final_schema.judge_audit.get("by_rule", []) if r.get("status") in ["adopted", "conditional"]],
                        "key_synthesis": final_schema.expert_version[:500] if final_schema.expert_version else "（决策逻辑待补充）",
                        "risks_and_boundaries": self._extract_risks_from_board(final_schema.board_version) or ["（风险分析待补充）"]
                    },
...
class FinalOutputSchema(BaseModel):
    """认知晶体树标准输出结构（对齐Excel九角色职责表）"""
    meta: Dict[str, str] = Field(default_factory=dict)
    
    # ===== 阶段1：各角色独立观点（对应Excel序号1-7）=====
    role_contributions: Dict[str, RoleViewpoint] = Field(
        default_factory=dict,
        description="7位辩论者的完整观点（每位200字）"
    )
    
    # ===== 轮次演进 =====
    round_by_round: List[RoundDynamics] = Field(default_factory=list)
    
    # ===== 阶段2：大法官输出（对应Excel第8项）=====
    judge_performance_board: List[Dict[str, Any]] = Field(
        default_factory=list,
        description="角色绩效看板：含角色、贡献度%、KPI综合分、状态、核心理由"
    )
    judge_final_verdict: str = Field(
        default="",
        description="终审裁决（200字左右）"
    )
    judge_rejected_details: str = Field(
        default="",
        description="驳回明细（100字左右）"
    )
    
    # ===== 阶段3：首席发言人输出（对应Excel第9项）=====
    spokesperson_narrative: str = Field(
        default="",
        description="首席发言人叙事（600字）：散文/演讲/故事风格"
    )
    
    # ===== 儒雅笔谈（附录，保留原有功能）=====
    elegant_epilogue: str = Field(
        default="",
        description="儒雅笔谈（附录）：苏轼/辛弃疾风格的散文"
    )
    
    # ===== 兼容层（保留旧字段，标记为deprecated）=====
    board_version: str = Field(default="", description="已废弃，请使用spokesperson_narrative")
    employee_version: str = Field(default="", description="已废弃")
    novice_version: str = Field(default="", description="已废弃")
    expert_version: str = Field(default="", description="已废弃")
    judge_audit: Dict[str, Any] = Field(default_factory=dict, description="已废弃，请使用judge_*字段")
    dashboard_stats: Dict[str, int] = Field(default_factory=dict, description="已废弃")
...
class OutputOrchestrator:
    ...
    def generate(self, question: str, debate_rounds: List[Dict]) -> FinalOutputSchema:
        """
        生成标准化的辩论输出（对齐Excel九角色职责表）
        
        流程：
        1. 提取原子数据（零AI）- 包含各角色完整观点
        2. 大法官裁决（AI）- 生成看板+终审+驳回
        3. 首席发言人叙事（AI）- 基于法官裁决生成
        4. 儒雅笔谈（AI）- 附录，保留原有风格
        """
        # ===== 阶段1：提取原子数据 =====
        ...
        # ===== 阶段2：大法官裁决 =====
        judge_result = self._run_judge(question, atomic, debate_rounds)
        
        # ===== 阶段3：首席发言人叙事 =====
        spokesperson_narrative = self._render_spokesperson(question, judge_result, atomic)
        
        # ===== 附录：儒雅笔谈（保留原有功能）=====
        elegant_epilogue = self._render_elegant_epilogue(question, judge_result, atomic)
        
        # ===== 轮次演进 =====
        round_by_round = self._extract_round_dynamics(debate_rounds)
        ...
        return FinalOutputSchema(
            meta={
                "question": question,
                "timestamp": datetime.now().isoformat(),
                "version": "2.2_excel_aligned"
            },
            role_contributions=atomic.get("role_contributions", {}),
            round_by_round=round_by_round,
            
            # ===== 阶段2：大法官输出 =====
            judge_performance_board=judge_result.get("performance_board", []),
            judge_final_verdict=judge_result.get("final_verdict", ""),
            judge_rejected_details=judge_result.get("rejected_details", ""),
            
            # ===== 阶段3：首席发言人叙事 =====
            spokesperson_narrative=spokesperson_narrative,
            
            # ===== 附录：儒雅笔谈 =====
            elegant_epilogue=elegant_epilogue,
            
            # ===== 兼容层 =====
            board_version=spokesperson_narrative[:300] if spokesperson_narrative else "",
            employee_version=spokesperson_narrative[:500] if spokesperson_narrative else "",
            novice_version=spokesperson_narrative[:200] if spokesperson_narrative else "",
            expert_version=judge_result.get("final_verdict", ""),
            judge_audit=judge_result,
            dashboard_stats=self._compute_dashboard_stats(judge_result)
        )
    ...
    def _extract_atomic(self, question: str, rounds: List[Dict]) -> Dict:
        ...
        保留每个角色的完整观点（200字），用于后续展示
        """
        # 标准角色顺序（对应Excel序号1-7）
        role_order = ["激进者", "保守者", "结构主义者", "百灵鸟", "取经者", "奇谋者", "延安智者"]
        ...
        for rd in rounds:
            for item in rd.get("answers", []):
                ...
                # 保存完整回答（如果还没保存或当前更长）
                if len(answer) > len(contributions[role]["full_answer"]):
                    contributions[role]["full_answer"] = answer
                
                # 提取观点段落（取前5段，每段不超过300字）
        ...
        # ===== 先处理百灵鸟裸模型：将其内容合并到百灵鸟中 =====
        bare_lark_content = None
        if "百灵鸟（裸模型）" in contributions:
            bare_lark = contributions["百灵鸟（裸模型）"]
            if bare_lark["viewpoints"]:
                bare_lark_content = "[裸模型基线] " + bare_lark["viewpoints"][0]
            # 如果有完整回答也保存
            if bare_lark["full_answer"] and not bare_lark_content:
                bare_lark_content = "[裸模型基线] " + bare_lark["full_answer"][:200]
        ...
        for name, data in contributions.items():
            ...
            # 如果观点不足，从完整回答中补充
            if not data["viewpoints"] and data["full_answer"]:
                sentences = re.split(r'[。！？；;]', data["full_answer"])
                for s in sentences[:3]:
                    s = s.strip()
                    if len(s) > 10:
                        data["viewpoints"].append(s[:300])
            
            # ===== 如果是百灵鸟，将裸模型内容作为额外观点添加 =====
            if name == "百灵鸟" and bare_lark_content:
                data["viewpoints"].append(bare_lark_content)
        ...
        for role_name in role_order:
            if role_name in result:
                sorted_result[role_name] = result[role_name]
        # 补全未在顺序中的角色
        for name, data in result.items():
            if name not in sorted_result:
                sorted_result[name] = data
        
        return {
            "role_contributions": sorted_result,
            "meta": {"question": question},
            "full_answers": {k: v["full_answer"] for k, v in contributions.items()}
        }

    def _map_role_key(self, name: str) -> str:
        mapping = {
            "激进者": "radical", "保守者": "conservative", "结构主义者": "structural",
            "百灵鸟": "lark", "取经者": "pilgrim", "奇谋者": "strategist",
            "延安智者": "statesman", "大法官": "judge", "首席发言人": "spokesperson",
            "替身-我": "twin"
        }
        return mapping.get(name, name)

    def _run_judge(self, question: str, atomic: Dict, rounds: List[Dict]) -> Dict:
        """
        大法官裁决 - 输出对齐Excel九角色职责表（第8项）
        
        输出结构：
        1. performance_board: 角色绩效看板（含贡献度%、KPI综合分、状态、核心理由）
        2. final_verdict: 终审裁决（200字）
        3. rejected_details: 驳回明细（100字）
        """
        # 构建各方观点摘要
        summary_parts = []
        for name, rv in atomic["role_contributions"].items():
            viewpoints = rv.viewpoints[:3] if rv.viewpoints else ["（未提供有效观点）"]
            evidence = rv.evidence_links[:3] if rv.evidence_links else ["无引用"]
            summary_parts.append(
                f"【{name}】\n"
                f"核心观点：{'; '.join(viewpoints)}\n"
                f"引用证据：{', '.join(evidence)}\n"
            )
        
        role_names = list(atomic["role_contributions"].keys())
        
        prompt = f"""你是认知晶体树的「大法官」。请对以下辩论做出终审裁决。
...
{{
  "performance_board": [
    {{
      "role": "角色名",
      "contribution_percent": 整数（0-100，所有角色总和必须=100）,
      "kpi_score": 小数（0-10，保留1位小数）,
      "status": "采纳" 或 "附条件" 或 "暂缓" 或 "驳回",
      "reason": "核心理由（20字以内）"
    }}
  ],
  "final_verdict": "终审裁决（200字左右）：先说明争议核心，再给出最终结论",
  "rejected_details": "驳回明细（100字左右）：列出被驳回的角色及具体原因"
}}

【评分维度】
- 论证扎实度（0-2.5分）：是否有充分证据和逻辑支撑
- 可落地性（0-2.5分）：方案是否在现实约束下可行
- 证据充分性（0-2.5分）：是否引用晶体/孔洞/外部来源
- 逻辑清晰度（0-2.5分）：推理链条是否完整、无跳跃

【状态判定规则】
- 采纳：论证扎实且可落地，直接纳入最终方案
- 附条件：有价值但需补充条件或成本论证
- 暂缓：方向正确但当前不紧迫，留待后续
- 驳回：颠覆性有余但不可落地，或逻辑跳跃风险不可控

【关键约束】
1. contribution_percent 总和必须 = 100
2. kpi_score 精确到0.1分
3. 每个角色都必须出现在 performance_board 中
4. final_verdict 必须包含「争议核心」和「最终结论」两部分
5. rejected_details 必须列出所有被驳回角色的具体驳回原因
"""
        try:
            res = self.ai.chat_json(prompt, temperature=0.3)
            judge_result = self._normalize_judge_output(res, role_names)
        except Exception as e:
            judge_result = self._generate_fallback_judge(atomic, role_names)
            print(f"[WARN] 法官裁决失败，使用降级方案: {e}")
        
        # ===== 更新突触权重 =====
        for item in judge_result.get("performance_board", []):
            role_name = item.get("role", "")
            status = item.get("status", "")
            role_key = self._map_role_key(role_name)
            if role_name in atomic.get("role_contributions", {}):
                rv = atomic["role_contributions"][role_name]
                for link in rv.evidence_links[:2]:
                    if status == "采纳":
                        SynapseStore.update_synapse(self.engine, role_key, link, 0.08)
                    elif status == "驳回":
                        SynapseStore.update_synapse(self.engine, role_key, link, -0.05)
        
        return judge_result
    def _normalize_judge_output(self, raw: Dict, role_names: List[str]) -> Dict:
        """强制规范化法官输出，确保符合Excel结构"""
        result = {
            "performance_board": [],
            "final_verdict": "",
            "rejected_details": ""
        ...
        # 1. 提取或生成 performance_board
        board = raw.get("performance_board", [])
        if not board:
            # 从旧格式迁移
            for item in raw.get("by_rule", []):
                board.append({
                    "role": item.get("target_role", "未知"),
                    "contribution_percent": 0,
                    "kpi_score": 7.0,
                    "status": item.get("status", "暂缓"),
                    "reason": item.get("verdict", "")[:20]
                })
        
        # 确保所有角色都在board中
        existing_roles = {item.get("role") for item in board}
        ...
        # 2. 强制 contribution_percent 总和 = 100
        total = sum(item.get("contribution_percent", 0) for item in board)
        if total != 100 and total > 0:
            for item in board:
                item["contribution_percent"] = round(item.get("contribution_percent", 0) / total * 100, 1)
        elif total == 0:
            per = round(100 / len(board), 1)
            for item in board:
                item["contribution_percent"] = per
            if board:
                board[0]["contribution_percent"] = 100 - per * (len(board) - 1)
        
        # 3. 确保kpi_score在0-10之间
        for item in board:
            score = item.get("kpi_score", 5.0)
            item["kpi_score"] = max(0.0, min(10.0, round(float(score), 1)))
        
        # 4. 确保状态字段合法
        valid_statuses = ["采纳", "附条件", "暂缓", "驳回"]
        status_map = {
            "adopted": "采纳",
            "conditional": "附条件",
            "deferred": "暂缓",
        ...
        for item in board:
            ...
            if status not in valid_statuses:
                item["status"] = status_map.get(status, "暂缓")
        
        result["performance_board"] = board
        
        # 5. 终审裁决
        final_verdict = raw.get("final_verdict", "")
        if not final_verdict or len(final_verdict) < 30:
            adopted = [item for item in board if item["status"] == "采纳"]
            if adopted:
                roles = "、".join([item["role"] for item in adopted[:3]])
                final_verdict = f"本案争议核心在于「{role_names[0] if role_names else '问题'}」的判断。经逐角色KPI评审，采纳{roles}的方案。最终裁决：以{adopted[0]['role']}的方案为主框架，结合其他角色有效观点，形成综合决策。"
            else:
                final_verdict = f"本案经{len(role_names)}位角色辩论，各方观点各有优劣。最终裁决：以保守者方案为基线，辅以结构主义者的框架优化，形成可落地的综合方案。"
        
        result["final_verdict"] = final_verdict[:300]
        
        # 6. 驳回明细
        rejected_details = raw.get("rejected_details", "")
        if not rejected_details:
            rejected = [item for item in board if item["status"] == "驳回"]
            if rejected:
                details = []
                for item in rejected:
                    details.append(f"• {item['role']}：{item.get('reason', '风险过高或不可落地')}")
                rejected_details = "\n".join(details)
            else:
                rejected_details = "所有角色均通过KPI审核，无驳回项。"
        
        result["rejected_details"] = rejected_details[:150]
        
        return result
    
    def _generate_fallback_judge(self, atomic: Dict, role_names: List[str]) -> Dict:
        """降级方案：当AI法官失败时，用规则生成裁决"""
        board = []
        total_roles = len(role_names)
        per = round(100 / total_roles, 1) if total_roles > 0 else 0
        
        for i, role in enumerate(role_names):
            rv = atomic.get("role_contributions", {}).get(role, {})
            evidence_count = len(rv.evidence_links) if hasattr(rv, 'evidence_links') else 0
            viewpoint_count = len(rv.viewpoints) if hasattr(rv, 'viewpoints') else 0
            
            kpi = 5.0 + min(2.5, evidence_count * 0.5) + min(2.5, viewpoint_count * 0.3)
            kpi = round(min(10.0, kpi), 1)
        ...
        adopted = [item for item in board if item["status"] == "采纳"]
        if adopted:
            final_verdict = f"经规则引擎裁决，采纳{', '.join([item['role'] for item in adopted[:3]])}的方案。"
        else:
            final_verdict = "经规则引擎裁决，建议以保守者方案为基线，结合各方意见形成综合方案。"
        
        rejected = [item for item in board if item["status"] == "驳回"]
        rejected_details = "\n".join([f"• {item['role']}：{item.get('reason', '风险过高')}" for item in rejected]) if rejected else "无驳回项。"
        
        return {
            "performance_board": board,
            "final_verdict": final_verdict,
            "rejected_details": rejected_details
        }

    def _render_spokesperson(self, question: str, judge_result: Dict, atomic: Dict) -> str:
        """
        首席发言人 - 基于法官裁决生成优美叙事（对齐Excel第9项）
        
        风格：散文/演讲/故事风格，600字左右
        """
        # 构建法官裁决摘要
        board_summary = []
        for item in judge_result.get("performance_board", []):
            ...
            board_summary.append(
                f"{status_icon} **{item['role']}**：{item.get('reason', '')}（KPI: {item.get('kpi_score', 0)}/10，贡献度 {item.get('contribution_percent', 0)}%）"
            )
        
        # 提取各角色核心观点（用于叙事素材）
        role_quotes = []
        for name, rv in atomic.get("role_contributions", {}).items():
            if rv.viewpoints:
                quote = rv.viewpoints[0][:80]
                role_quotes.append(f"{name}：{quote}...")
        
        prompt = f"""你是认知晶体树的「首席发言人」。你的使命是：将大法官的裁决转化为精彩、动人、有吸引力的叙事。

【用户问题】
{question}

【大法官裁决摘要】
{chr(10).join(board_summary)}

【各角色核心观点（素材）】
{chr(10).join(role_quotes[:7])}

【终审裁决】
{judge_result.get('final_verdict', '')}

【驳回明细】
{judge_result.get('rejected_details', '')}

【输出要求】
1. 风格：散文/演讲/故事风格，读来如沐春风，心中舒畅
2. 结构：
   - 开篇：用一句有画面感的话引入
   - 中段：逐一评价各角色的贡献（采纳的赞美，驳回的温和否定）
   - 结尾：一句精炼的结论
3. 字数：600字左右（不少于500字，不超过700字）
4. 禁止：不要使用Markdown格式，不要分段标题，直接输出流畅叙事

【参考风格】
以"当延安智者画出'利益重构+财务翻译'的路线图时，这场辩论实际上已经结束了"这样的笔法开篇，全文保持这种从容而有力量的语调。
"""
        try:
            narrative = self.ai.chat(prompt, temperature=0.6)
            if len(narrative) < 300:
                narrative += "\n\n" + judge_result.get('final_verdict', '')
            return narrative[:800]
        except Exception as e:
            fallback = judge_result.get('final_verdict', '')
            if len(fallback) < 200:
                fallback = f"本案围绕「{question}」展开辩论。经评审，{judge_result.get('performance_board', [{}])[0].get('role', '多方')}的方案被采纳。最终结论：以实证为基础，以可落地为准则，形成综合决策。"
            return fallback

    def _render_elegant_epilogue(self, question: str, judge_result: Dict, atomic: Dict) -> str:
        """
        儒雅笔谈（附录）- 苏轼/辛弃疾风格的散文
        ...
        one_sentence = final_verdict[:50] if final_verdict else question[:30]
        
        # 提取采纳角色
        adopted = [item for item in judge_result.get("performance_board", []) if item.get("status") == "采纳"]
        adopted_names = "、".join([item['role'] for item in adopted[:3]]) if adopted else "诸位"
        
        prompt = f"""
请将以下内容改写为一段「儒雅风格」的文字。
...
3. 引用一句古诗词或化用其意境
4. 结尾落在"行"字上——不是空谈，是可行之道
5. 字数：80-120字（精简隽永，点到即止）
6. 只输出正文，不加标题

【输出要求】
只输出正文，不加标题、不加序号、不加任何格式标记。
"""
        try:
            return self.ai.chat(prompt, temperature=0.5)
        except Exception as e:
            return f"以我观之，此事如月照寒潭，明澈而深邃。{one_sentence}。行者自知，行之者自达。"

    def _extract_round_dynamics(self, rounds: List[Dict]) -> List[RoundDynamics]:
        """从原始辩论中提取轮次演进关系"""
        round_list = []
        for rd in rounds:
            round_no = rd.get("round", 0)
            ...
            for item in answers:
                role = item.get("role", "")
                answer = item.get("answer", "")
                if "百灵鸟" not in role and "大法官" not in role:
            ...
            round_list.append(RoundDynamics(
                round=round_no,
                dynamics=dynamics if dynamics else "（本轮辩论进行中）",
                absorptions=absorptions[:3]
            ))
        
        return round_list

    def _compute_dashboard_stats(self, judge_result: Dict) -> Dict[str, int]:
        """计算看板统计"""
        board = judge_result.get("performance_board", [])
        stats = {
            "采纳": 0,
            "附条件": 0,
            "暂缓": 0,
    ...
    def _run_spokesperson(self, question: str, rounds: List[Dict], judge: Dict) -> Dict:
        ...
        for rd in rounds:
            ...
            # 提取本轮核心焦点（取第一个角色的回答前100字）
            dynamics = ""
            for item in answers:
                role = item.get("role", "")
                answer = item.get("answer", "")
                if "百灵鸟" not in role:  # 跳过百灵鸟，用核心角色
                    dynamics = answer[:150].replace("\n", " ")
                    break
...
def apply_v3_patches():
    ...
    # 2. 创建进化日志文件
    evo_file = synapse_dir / "evolution_log.json"
    if not evo_file.exists():
        with open(evo_file, "w", encoding="utf-8") as f:
            json.dump({"events": [], "summary": {}}, f, ensure_ascii=False, indent=2)
        print(f"✅ 已创建进化日志文件: {evo_file}")
    else:
        print(f"✅ 进化日志文件已存在: {evo_file}")
