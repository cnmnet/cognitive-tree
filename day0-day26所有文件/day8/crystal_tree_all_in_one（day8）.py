...
class CrystalTreeApp:
    def __init__(self):
        ...
        self._chat_in_progress = False  # ← 新增防重复标志
        self._suppress_select = False   # 用于抑制自动触发的选择事件
...
{final}"""
    ...
    def _debate_engine_reasoning(self, user_input: str, debate_mode: str, label: str, max_rounds: int = 2):
        try:
            ...
            # ================================================================
            # 从 final_schema 读取数据（不再使用 rigid）
            # ================================================================
            ...
            # 2. 提取核心结论（从 board_version 取第一句）
            import re
            one_sentence = ""
            if board_version:
                match = re.search(r'^[^。！？]*[。！？]', board_version)
                one_sentence = match.group(0) if match else board_version[:80] + "..."

            # 3. 提取风险（从 board_version 中提取）
            risks = []
            if board_version:
                risk_match = re.search(r'风险[：:]\s*(.+?)(?:\n|$)', board_version)
                if risk_match:
                    risks = [risk_match.group(1).strip()]
                else:
                    redline_match = re.search(r'红线[：:]\s*(.+?)(?:\n|$)', board_version)
                    if redline_match:
                        risks = [redline_match.group(1).strip()]

            # 4. 提取决策逻辑（从 expert_version 取前500字）
            key_synthesis = ""
            if expert_version:
                key_synthesis = expert_version[:500]
                if len(expert_version) > 500:
                    key_synthesis += "……"

            # 5. 提取各版本内容
            student_answer = novice_version or "（通俗解读待生成）"
            teacher_detail = expert_version or "（详细复盘待生成）"
            soft_wrap = employee_version or "（精简版待生成）"

            # 6. 提取采纳项（从 judge_audit 中提取）
            core_adoptions = []
            for rule in judge_audit.get("by_rule", []):
                if rule.get("status") in ["adopted", "conditional"]:
                    target = rule.get("target_role", "")
                    verdict = rule.get("verdict", "")
                    if target and verdict:
                        core_adoptions.append(f"{target}：{verdict}")

            # 7. 构建各角色核心观点（遍历所有轮次，取每个角色的第一次发言）
            role_blocks = []
            rounds_data = result.get("rounds", [])
            seen_roles = set()
            for rd in rounds_data:
                for ans in rd.get("answers", []):
                    ...
                    # 只取前 300 字
                    if len(content) > 300:
                        content = content[:300] + "……（完整版见上方滚动日志）"
                    role_blocks.append(f"**{role_name}**：{content}")
                    # 如果已经收集了所有角色，提前退出
                    if len(seen_roles) >= 12:  # 最多 12 个角色
                        break
                if len(seen_roles) >= 12:
                    break

            role_views_text = "\n\n".join(role_blocks) if role_blocks else "（无独立观点记录）"
...
【3. 核心结论】
{one_sentence if one_sentence else '（核心结论待生成）'}

【4. 决策逻辑与关键融合】
{key_synthesis if key_synthesis else '（决策逻辑待生成，请查看专家版详情）'}

【5. 风险与边界】
{chr(10).join('- ' + str(item) for item in risks) if risks else '（暂未识别出明确风险）'}

【6. 详细复盘与最终裁决逻辑（教师版）】
{teacher_detail if teacher_detail else '（详细复盘待生成）'}

【7. 通俗解读（精简版）】
{soft_wrap if soft_wrap else student_answer}

【8. 法官裁决依据】
{chr(10).join([f"- {r.get('target_role','')}：{r.get('verdict','')}（{r.get('status','')}）" for r in judge_audit.get('by_rule', [])]) if judge_audit.get('by_rule') else '（无裁决记录）'}

【9. 看板数据】
采纳: {dashboard_stats.get('adopted', 0)} | 附条件: {dashboard_stats.get('conditional', 0)} | 暂缓: {dashboard_stats.get('deferred', 0)} | 驳回: {dashboard_stats.get('rejected', 0)}
"""

            # 9. 生成儒雅笔谈
            elegant_text = self._generate_elegant_narrative(
                question=user_input,
...
class FinalOutputSchema(BaseModel):
    meta: Dict[str, str]
    role_contributions: Dict[str, RoleViewpoint]
    judge_audit: Dict[str, Any]
    round_by_round: List[RoundDynamics]
    board_version: str = Field(..., min_length=20, max_length=500)
    employee_version: str = Field(..., min_length=100, max_length=3200)
    novice_version: str = Field(..., min_length=50, max_length=1500)
    expert_version: str = Field(..., min_length=100, max_length=4000)
    elegant_epilogue: str = ""
...
class OutputOrchestrator:
    ...
    def generate(self, question: str, debate_rounds: List[Dict]) -> FinalOutputSchema:
        ...
        # ===== 渲染五个版本 =====
        # 1. 老板版 - 需要完整辩论数据
        board = self._render_board(question, debate_rounds, judge_result)
    ...
    def _run_judge(self, question: str, atomic: Dict) -> Dict:
        ...
        for name, rv in atomic["role_contributions"].items():
            summary.append(f"{name}: {'; '.join(rv.viewpoints[:2])}")

        prompt = f"""
你是「大法官」。请裁决以下辩论，**必须引用具体晶体ID（如C012）或孔洞ID（H003）作为依据**。
...
{{
  "by_rule": [
    {{"rule": "引用的晶体ID", "target_role": "角色名", "verdict": "结论", "status": "adopted|conditional|deferred|rejected"}}
  ],
  "final_verdict": "终审结论",
  "rejected_items": [{{"item": "被驳回内容", "reason": "理由"}}]
}}
"""
        try:
            res = self.ai.chat_json(prompt, temperature=0.3)
            judge_result = res if "error" not in res else {"by_rule": [], "final_verdict": "裁决失败"}
        except Exception:
            judge_result = {"by_rule": [], "final_verdict": "裁决异常"}

        # ===== 新增：更新突触权重 =====
        for rule in judge_result.get("by_rule", []):
            role_name = rule.get("target_role", "")
            ...
            SynapseStore.update_synapse(self.engine, role_key, rule_id, delta)
