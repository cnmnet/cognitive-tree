...
"""
    ...
    def run(self, question: str, mode: str = "debate_full", max_rounds: int = 4) -> Dict:
        ...
        # Day 4: 非马尔可夫历史检索（诊断历史经验）
        try:
            history_result = self.engine.meta.diagnose_history(question, threshold=0.7)
            self._history_result = history_result
            if history_result.get("matched", False):
                self._reused_crystals = history_result.get("crystal_combination", [])
                match_score = history_result.get("match_score", 0)
                history_question = history_result.get("matched_question", "")
                self.log(
                    f"📚 复用历史经验：匹配历史问题（相似度 {match_score:.2f}）：{history_question[:60]}...",
                    "system"
                )
                self.log(
                    f"  有效晶体组合：{', '.join(self._reused_crystals) if self._reused_crystals else '无'}",
                    "system"
                )
                # 记录复用事件
                self.engine.log_evolution_event(
                    ...
                    {
                        "question": question,
                        "matched_question": history_question,
                        "match_score": match_score,
                        "reused_crystals": self._reused_crystals,
                        "trigger": "debate_init"
                    }
                )
            else:
        ...
        try:
            ...
            self.log(f"🧠 使用默认认知风格：{self._cognitive_operators}", "system")
        # Day 2.8: 元问题分类
            ...
            for alarm in triggered:
                ...
                if alarm["rule"] == "knowledge_poverty":
                    self._record_failure_trace(
                        ...
                        {
                            "ref_rate": metrics["crystal_reference_rate"],
                            "threshold": 0.5,
                            "round": round_no,
                            "effective_crystals": [c for c in self._reused_crystals] if hasattr(self, '_reused_crystals') else []
                        }
                    )
        ...
        # ===== 输出真实的Token消耗统计 =====
        try:
            real_tokens = self.ai._token_estimate
            real_cost = real_tokens * 0.000001  # DeepSeek 约 $1/百万token
            self.log(f"📊 真实Token消耗：{real_tokens} tokens，预估成本：${real_cost:.6f}", "ai")
            self.log(f"📊 预算使用率：{real_tokens / routing_result.get('token_budget', 1) * 100:.1f}%", "ai")
        except Exception as e:
            self.log(f"⚠️ Token统计失败：{e}", "warning")
    ...
    def _spokesperson(self, question: str, rounds: List[Dict], crystal_context: str, light: bool = False) -> Dict:
        ...
        # ===== 插入认知风格指令（关键修改） =====
        style_instruction = ""
        if hasattr(self, '_cognitive_operators') and self._cognitive_operators:
            style_instruction = f"""
【用户认知风格偏好】{self._cognitive_operators}
请根据这些偏好调整最终答案的表达方式：
- 如果偏好“先结论后展开”，则答案第一段直接给结论；
- 如果偏好“类比思维”，则多用比喻和类比；
- 如果偏好“归纳推理”，则先给案例再总结；
- 如果偏好“演绎推理”，则从一般原理推导到具体。
"""
...
{{
  ...
  "teacher_detail": "给老师看的辩论过程摘要和审计依据，**必须 1200~2000 字**，并包含以下结构：\n1. 轮次复盘（每轮各方核心观点变化）\n2. 证据评分表（各角色最终证据评分及理由）\n3. 最终裁决逻辑（**必须详细展开，至少 500 字**，明确说明：① 采纳哪一方的核心立场，依据了哪些具体证据（引用晶体ID或案例）；② 放弃哪一方的立场，具体理由是什么；③ 如果存在混合方案，说明各方的贡献比例及权衡依据。）",
...
"""
        ...
        try:
            ...
            self.log("[WARN] JSON 解析失败，尝试提取关键信息", "warning")
            # 尝试从 raw_text 中提取关键字段（如果它本身就是 JSON 字符串）
            try:
                ...
                # 用正则提取常见字段
                import re
                decision = re.search(r'"decision_summary"\s*:\s*"([^"]*)"', cleaned)
                synthesis = re.search(r'"key_synthesis"\s*:\s*"([^"]*)"', cleaned)
                student = re.search(r'"student_friendly_answer"\s*:\s*"([^"]*)"', cleaned)
                teacher = re.search(r'"teacher_detail"\s*:\s*"([^"]*)"', cleaned)
                soft = re.search(r'"soft_wrap"\s*:\s*"([^"]*)"', cleaned)
                one = re.search(r'"one_sentence_conclusion"\s*:\s*"([^"]*)"', cleaned)

                # 如果至少提取到核心信息，则组装返回
                if decision or synthesis or student:
                    return {
                        "rigid_core": {
                            "decision_summary": decision.group(1) if decision else "（无法提取）",
                            "core_adoptions": [],
                            "key_synthesis": synthesis.group(1) if synthesis else "（无法提取）",
                            "risks_and_boundaries": []
                        },
                        "soft_wrap": soft.group(1) if soft else "（无法提取）",
                        "one_sentence_conclusion": one.group(1) if one else "（无法提取）",
                        "student_friendly_answer": student.group(1) if student else "（无法提取）",
                        "teacher_detail": teacher.group(1) if teacher else "（无法提取）",
                        "pending_card_suggestion": ""
                    }
            except:
                pass

            # 最终降级：生成友好的中文错误提示，而不是直接显示 JSON 原文
            return {
                "rigid_core": {
                    "decision_summary": "AI 返回的格式异常，无法解析为结构化数据。以下为原始响应片段（仅供参考）：" + raw_text[:200],
                    "core_adoptions": [],
                    "key_synthesis": "（格式异常，无法生成融合方案）",
                    "risks_and_boundaries": ["系统无法解析本次辩论结果，请检查 AI 响应格式。"]
                },
                "soft_wrap": "抱歉，本次辩论总结出现格式错误，未能生成完整的通俗解读。建议重新提问或检查 API 配置。",
                "one_sentence_conclusion": "格式错误，请重试。",
                "student_friendly_answer": "（内容生成异常，无法展示学生版答案）",
                "teacher_detail": "（内容生成异常，无法展示教师版复盘）",
                "pending_card_suggestion": ""
            }
...
{final}"""
    ...
    def _debate_engine_reasoning(self, user_input: str, debate_mode: str, label: str, max_rounds: int = 2):
        try:
            ...
            # 提取第1轮各角色原始观点
            role_blocks = []
            first_round_data = result.get("rounds", [])[0] if result.get("rounds") else None
            if first_round_data:
                for ans in first_round_data.get("answers", []):
...
【5. 风险与边界】
{chr(10).join('- ' + str(item) for item in risks) if risks else '无'}

【6. 详细复盘与最终裁决逻辑（教师版）】
{teacher_detail if teacher_detail else '（无详细复盘）'}
