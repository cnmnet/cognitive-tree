...
"""
晶体→Skill 迁移脚本
将现有所有晶体卡片迁移到 skills/ 目录下的 Skill 子目录格式
"""
...
class CrystalToSkillMigrator:
    ...
    def parse_crystals_from_md(self) -> list:
        """从 Markdown 文件中解析晶体数据"""
        if not self.crystals_file.exists():
            print(f"警告: 晶体卡片文件不存在: {self.crystals_file}")
            return []
...
''')
        ...
        if input_conditions:
            ...
            for c in input_conditions:
                cond_checks.append(f'    if "{c}" not in str(input_data): print(f"  WARN: missing input condition: {c}")')
            test_functions.append(f'''
def test_input_conditions():
    """验证输入条件（宽松模式）"""
    input_data = {{"text": "test input", "用户输入问题": "这是一个测试问题"}}
{chr(10).join(cond_checks)}
...
'''
    ...
    def generate_crystal_md(self, crystal: dict, layer_state: dict) -> str:
        """生成 CRYSTAL.md 文件 - 使用字符串拼接避免三引号嵌套"""
        crystal_id = crystal["id"]
        content = crystal["content"]
        ...
        # 使用列表拼接构建 CRYSTAL.md 内容，避免三引号嵌套
        lines = []
    ...
    def migrate_crystal(self, crystal: dict, layer_state: dict) -> dict:
        ...
        # 设置可执行权限（Unix）
        if os.name != 'nt':
            os.chmod(skill_dir / "validate.py", 0o755)

        # 生成 references/ 下的引用文件（如果有链接）
        refs_dir = skill_dir / "references"
        for link in crystal.get("links", []):
            ref_file = refs_dir / f"{link}.md"
            if not ref_file.exists():
                ref_file.write_text(f"# 引用: {link}\n\n此文件是对晶体 {link} 的引用占位。\n\n请根据需要补充内容。", encoding='utf-8')

        # 生成 README.md
        readme = skill_dir / "README.md"
        if not readme.exists():
            readme.write_text(f"# {crystal_id} Skill\n\n此 Skill 包含晶体 {crystal_id} 的完整定义和验证脚本。\n\n## 文件结构\n\n- `CRYSTAL.md` - 晶体定义\n- `validate.py` - 验证脚本\n- `references/` - 外部引用\n", encoding='utf-8')
    ...
    def run(self) -> dict:
        ...
        for crystal in crystals:
            try:
                ...
                print(f"  OK {crystal['id']} -> {result['path']}")
            except Exception as e:
                print(f"  FAIL {crystal['id']} 迁移失败: {e}")
    ...
    def generate_migration_report(self, results: list, failed: list) -> str:
        """生成迁移报告"""
        report_path = self.data_root / "系统日志" / "晶体迁移报告.md"

        lines = []
        lines.append("# 晶体->Skill 迁移报告")
        lines.append("")
...
def main():
    ...
    if result["success"]:
        print("\n所有晶体迁移成功！")
        print("   skills/ 目录已创建，包含所有 Skill 子目录")
        print("   每个 Skill 目录包含 CRYSTAL.md 和 validate.py")
    else:
        print(f"\n迁移部分失败: {result['failed']} 条")
        print("   请检查错误详情并重试")
