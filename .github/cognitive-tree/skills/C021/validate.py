...
# -*- coding: utf-8 -*-
"""
C021 validation script
Auto-generated during crystal migration

Crystal content: 负反馈强制触发原则...
"""
...
def test_input_conditions():
    ...
    if "系统出现持续偏差" not in str(input_data): print(f"  WARN: missing input condition: 系统出现持续偏差")
    if "反馈回路失效" not in str(input_data): print(f"  WARN: missing input condition: 反馈回路失效")
    if "需要强制纠正行为" not in str(input_data): print(f"  WARN: missing input condition: 需要强制纠正行为")
...
def test_validation_criteria():
    """验证所有验证标准"""
    print("  criteria 1: 偏差在预期时间内消除")
    print("  criteria 2: 系统未出现震荡")
    print("  criteria 3: 强制信号未造成次生损害")
    print("[PASS] validation_criteria")


def main():
    """Run all validation tests"""
    print(f"Validating crystal C021...")
    print("-" * 50)
