...
window.loadFingerprint = loadFingerprint;
  ...
  async function handleLogin() {
    ...
    try {
      ...
      hideLoginOverlay();
      appendSystem(`✅ 欢迎回来，${currentUser.username}！`, 'success');
      updateAuthUI();
    } catch (e) {
  ...
  function updateAuthUI() {
    ...
    if (currentUser) {
      ...
      if (remaining !== undefined) {
        const badge = document.querySelector('.badge-date');
        if (badge) {
          badge.innerHTML = `<i class="far fa-calendar-alt"></i> 试用剩余 ${remaining} 次`;
        }
      }
    } else {
