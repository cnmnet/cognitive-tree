...
{final}"""
    ...
    def _debate_engine_reasoning(self, user_input: str, debate_mode: str, label: str, max_rounds: int = 2):
        ...
        try:
            ...
            # ===== 调试日志 =====
            print(f"[DEBUG] ========== 渲染前检查 ==========")
            print(f"[DEBUG] final_schema 类型: {type(final_schema)}")
            print(f"[DEBUG] final_schema 是否为空: {not final_schema}")
            if final_schema:
                print(f"[DEBUG] final_schema keys: {list(final_schema.keys())}")
            print(f"[DEBUG] board_version 长度: {len(board_version)}")
            print(f"[DEBUG] employee_version 长度: {len(employee_version)}")
            print(f"[DEBUG] novice_version 长度: {len(novice_version)}")
            print(f"[DEBUG] expert_version 长度: {len(expert_version)}")
            print(f"[DEBUG] result_area 是否存在: {self.result_area is not None}")
...
"""
    ...
    def _render_elegant(self, judge_result: Dict) -> str:
        ...
        verdict = judge_result.get("final_verdict", "")
        if verdict:
            return f"以我观之，此事如月照寒潭，明澈而深邃。{verdict[:50]}。行者自知，行之者自达。"
        return "以我观之，此事如月照寒潭，明澈而深邃。行者自知，行之者自达。"
