...
def list_market_skills():
    try:
        market = engine.get_skill_market()
        skills = market.list_skills()
        return {"status": "success", "skills": skills}
    except Exception as e:
        return {"status": "error", "error": str(e)}

@app.post("/api/skill-market/install/{skill_id}")
def install_skill(skill_id: int):
    try:
        market = engine.get_skill_market()
        result = market.install_skill(skill_id)
...
class GitHubSkillMarket:
    """从 GitHub Issues 读取 Skill 包信息，支持一键安装"""
    ...
    def list_skills(self, label: str = "skill") -> List[Dict]:
        """列出所有标记为 skill 的 Issue"""
        ...
        for issue in issues:
            ...
            if metadata:
                skills.append({
                    "id": issue["number"],
                    "title": issue["title"],
                    "author": metadata.get("作者", "unknown"),
                    "description": metadata.get("描述", ""),
                    "price": metadata.get("定价", "免费"),
                    "url": issue["html_url"],
    ...
    def _parse_metadata(self, body: str) -> Dict:
        """从 Issue body 解析 key: value 对"""
        lines = body.split("\n")
        data = {}
        for line in lines:
            if ":" in line:
    ...
    def install_skill(self, skill_id: int, target_dir: str = "skills") -> Dict:
        """从 Issue 安装 Skill 到本地"""
        # 1. 获取 Issue 详情
        url = f"{self.api_base}/repos/{self.owner}/{self.repo}/issues/{skill_id}"
        resp = requests.get(url, headers=self._headers())
        if resp.status_code != 200:
            return {"success": False, "error": "Issue not found"}
        issue = resp.json()
        body = issue.get("body", "")
        # 2. 提取 Skill 包下载地址（可能来自附件或评论）
        # 这里简化：假设 issue body 中包含 ```python 代码块
        import re
        ...
        # 3. 创建 skill 目录并写入文件
        from pathlib import Path
        skill_dir = Path(target_dir) / f"SKILL-{skill_id}"
        skill_dir.mkdir(parents=True, exist_ok=True)
        # 将代码块写入 validate.py
        (skill_dir / "validate.py").write_text(code_blocks[0], encoding="utf-8")
        # 写入 CRYSTAL.md（从 Issue 内容提取）
        crystal_content = f"# {issue['title']}\n\n{body}"
        (skill_dir / "CRYSTAL.md").write_text(crystal_content, encoding="utf-8")
