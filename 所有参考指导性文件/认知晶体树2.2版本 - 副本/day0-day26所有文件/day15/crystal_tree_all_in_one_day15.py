...
class ForceExplorer:
    ...
    def _create_skill_from_crystal(self, crystal_id: str, content: str, hole_id: str) -> bool:
        ...
        try:
            ...
            # 1. 生成 CRYSTAL.md（使用列表拼接，避免三引号嵌套）
            ...
            md_lines.append("- **来源**: 强制探索 (Day 11)")

            crystal_md.write_text("\n".join(md_lines), encoding='utf-8')

            # 2. 生成 validate.py（使用列表拼接）
            validate_py = skill_dir / "validate.py"
            py_lines = []
            py_lines.append('#!/usr/bin/env python3')
            py_lines.append('# -*- coding: utf-8 -*-')
            py_lines.append('"""')
            py_lines.append(f'{crystal_id} validation script')
            py_lines.append('Auto-generated by force exploration (Day 11)')
            py_lines.append('"""')
            py_lines.append('')
            py_lines.append('import sys')
            py_lines.append('')
            py_lines.append('')
            py_lines.append('def test_content_not_empty():')
            py_lines.append('    """验证晶体内容非空"""')
            py_lines.append(f'    content = "{content[:50]}..."')
            py_lines.append('    assert len(content) > 0, "crystal content cannot be empty"')
            py_lines.append('    print("[PASS] content_not_empty")')
            py_lines.append('')
            py_lines.append('')
            py_lines.append('def main():')
            py_lines.append('    """运行所有验证测试"""')
            py_lines.append(f'    print(f"Validating crystal {crystal_id}...")')
            py_lines.append('    print("-" * 50)')
            py_lines.append('    test_content_not_empty()')
            py_lines.append('    print("-" * 50)')
            py_lines.append('    print("All validation passed!")')
            py_lines.append('    return 0')
            py_lines.append('')
            py_lines.append('')
            py_lines.append('if __name__ == "__main__":')
            py_lines.append('    sys.exit(main())')
            ...
            readme_lines.append("")
            readme_lines.append("## 关联孔洞")
            readme_lines.append("")
            readme_lines.append(f"- {hole_id}")

            readme.write_text("\n".join(readme_lines), encoding='utf-8')

            # ===== 2. 同步更新晶体卡片.md =====
            # 读取现有内容
            crystals_file = self.engine.files.resolve("crystals")
            if crystals_file.exists():
                existing_content = crystals_file.read_text(encoding='utf-8')
            else:
                existing_content = "# 晶体卡片库\n\n| ID | 内容 | 链接 | 输入条件 | 执行逻辑 | 输出格式 | 验证标准 |\n|----|------|------|---------|---------|---------|---------|\n"

            # 检查是否已存在（避免重复）
            if f"| {crystal_id} |" in existing_content:
                self.log(f"  ⚠️ 晶体 {crystal_id} 已存在于晶体卡片.md，跳过同步", "warning")
            else:
                # 追加新行
                new_line = f"\n| {crystal_id} | {content} | {hole_id} | — | — | — | — |\n"
                self.engine.files.append("crystals", new_line)
                self.log(f"  📝 同步更新晶体卡片.md: {crystal_id}", "system")
            return True
        except Exception as e:
            self.log(f"  ❌ 创建 Skill 失败: {e}", "error")
            return False
...
"""
            try:
                ...
                if new_crystal_content and len(new_crystal_content) > 10:
                    ...
                    success = self._create_skill_from_crystal(new_id, new_crystal_content[:80], hole_id)
...
class CrystalEngine:
    ...
    def _create_skill_from_crystal(self, crystal_id: str, content: str, links: List[str] = None,
        ...
        try:
            ...
            py_lines.append('"""')
            py_lines.append(f'{crystal_id} validation script')
            py_lines.append('Auto-generated by crystal system')
            py_lines.append('"""')
            py_lines.append('')
            py_lines.append('import sys')
            ...
            py_lines.append('')
            py_lines.append('')
            py_lines.append(f'CONTENT = r"""{content}"""')
            py_lines.append('')
            py_lines.append('')
            py_lines.append('def test_content_not_empty():')
            py_lines.append('    """验证晶体内容非空"""')
            py_lines.append('    assert len(CONTENT) > 10, "晶体内容太短或为空"')
            py_lines.append('    print("[PASS] content_not_empty")')
            py_lines.append('')
            py_lines.append('')
            py_lines.append('def test_has_valid_structure():')
            py_lines.append('    """验证晶体结构是否完整"""')
            py_lines.append('    meaningful_words = ["认知", "原则", "框架", "模型", "决策", "系统", "策略", "方法", "分析", "评估"]')
            py_lines.append('    has_meaning = any(w in CONTENT for w in meaningful_words)')
            py_lines.append('    if not has_meaning:')
            py_lines.append('        print(f"[WARN] 晶体内容可能缺乏核心认知词汇")')
            py_lines.append('    print("[PASS] structure_check")')
            py_lines.append('')
            py_lines.append('')
            py_lines.append('def main():')
            py_lines.append('    """运行所有验证测试"""')
            ...
            py_lines.append('    print("-" * 50)')
            py_lines.append('    test_content_not_empty()')
            py_lines.append('    test_has_valid_structure()')
            py_lines.append('    print("-" * 50)')
            py_lines.append('    print("All validation passed!")')
            py_lines.append('    return 0')
            py_lines.append('')
...
class CrystalTreeApp:
    def __init__(self):
        self.files = FileIO()
        self.db = DBManager()
        self.ai = AIClient()  # 先创建 AI 客户端
        self.engine = CrystalEngine(self.files, ai_client=self.ai)  # 传递
...
class ClaimExtractor:
    ...
    def _extract_threshold_claims(self, text: str) -> List[VerifiableClaim]:
        ...
        # 增强的模式：X 高于/低于/大于/小于/超过/不足/不超过/至少 Y
        patterns = [
            r'([^，,。.；;]+?)\s*(?:高于|低于|大于|小于|超过|不足|不超过|不少于|至少|最多)\s*(\d+\.?\d*%?)',
            r'(?:成本|价格|费用|预算|支出)\s*(?:低于|超过|不足|不超过|至少|最多)\s*(\d+\.?\d*)\s*(?:元|美元|欧元|人民币)',
            r'(?:准确率|成功率|有效率|覆盖率)\s*(?:不低于|至少|超过)\s*(\d+\.?\d*%?)',
        ]
        ...
        for pattern in patterns:
            ...
            for match in matches:
                ...
                claim = VerifiableClaim(
                    claim_id=f"CLAIM-{self._claim_counter + 1:04d}",
                    original_text=f"{entity}阈值{value_str}",
        ...
        # 特别处理"低于/超过"等关键词（不依赖实体）
        threshold_patterns = [
            r'(低于|超过|不足|不超过|不少于|至少|最多)\s*(\d+\.?\d*%?)',
        ]
        for pattern in threshold_patterns:
            ...
            for match in matches:
                if len(match) == 2:
                    ...
                    claim = VerifiableClaim(
                        claim_id=f"CLAIM-{self._claim_counter + 1:04d}",
                        original_text=f"{keyword}{value_str}",
                        claim_type="threshold",
                        entity_a="指标",
                        value=value
                    )
    ...
    """
    # TODO: 实现具体的验证逻辑
    # 示例：从数据源获取 entity_a 和 entity_b 的值
