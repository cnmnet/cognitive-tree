...
{final}"""
    ...
    def _output_debate_report(self, question: str, result: Dict, board_version: str,
        ...
        # 从 result 中提取各角色观点（取第一轮发言，200字以内）
        rounds_data = result.get("rounds", [])
        role_views = {}
        ...
        for role in role_order:
            if role in role_views:
                self.root.after(100, lambda r=role, c=role_views[role]: self._log(f"▸ {r}：{c}", "ai"))
                self.root.after(100, lambda: self._log("", "system"))
        ...
        if elegant_epilogue:
            ...
            self.root.after(500, lambda: self._insert_foldable_content(
                elegant_epilogue,
                f"elegant_{int(time.time())}"
            ))
            self.root.after(500, lambda: self._log("", "system"))
        
        # ============================================================
        # 6. 报告结束
        # ============================================================
        self.root.after(550, lambda: self._log("=" * 75, "system"))
        self.root.after(550, lambda: self._log(f"✅ {label} 完成！", "success"))
        self.root.after(550, lambda: self._log("=" * 75, "system"))    
...
"""
    ...
    def _render_expert(self, question: str, debate_rounds: List[Dict], atomic: Dict, judge_result: Dict) -> str:
        ...
        for item in scorecard:
            matrix_lines.append(
                f"| {item.get('role', '未知')} | {item.get('strength', 5)} | {item.get('novelty', 5)} | "
...
【驳回明细】
{chr(10).join([f"- {item.get('item', '')}: {item.get('reason', '')}" for item in rejected]) if rejected else "无驳回项"}
