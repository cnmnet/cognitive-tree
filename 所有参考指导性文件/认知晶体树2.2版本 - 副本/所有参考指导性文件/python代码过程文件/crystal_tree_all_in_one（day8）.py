...
class OutputOrchestrator:
    ...
    def generate(self, question: str, debate_rounds: List[Dict]) -> FinalOutputSchema:
        ...
        # 从 judge_result 中提取所需字段
        judge_performance_board = judge_result.get("role_scorecard", [])   # 用于看板
        judge_final_verdict = judge_result.get("final_verdict", "")
        rejected_items = judge_result.get("rejected_items", [])
        judge_rejected_details = "\n".join([f"- {item.get('item', '')}: {item.get('reason', '')}" for item in rejected_items])
    ...
    def _run_judge(self, question: str, atomic: Dict) -> Dict:
        ...
        兼容 by_rule 和 role_scorecard 两种格式
        """
...
【输出要求】
请对以下7位辩论者逐一打分：
激进者、保守者、结构主义者、百灵鸟、取经者、奇谋者、延安智者
...
【输出格式】只返回JSON，不要Markdown：
{{
  "role_scorecard": [
    {{
      "role": "激进者",
      "core_view": "核心观点摘要（30字内）",
      "strength": 8, "novelty": 9, "feasibility": 4,
      "evidence_quality": 6, "relevance": 7, "alignment": 5, "activation": 8,
...
"""
        try:
            ...
            if "error" in res:
                print(f"[ERROR] _run_judge 返回错误: {res.get('error')}")
                return self._empty_judge_result()
            
            # ================================================================
            # 关键修复1：格式兼容层 - 支持 by_rule 和 role_scorecard 两种格式
            # ================================================================
            scorecard = res.get("role_scorecard", [])
            
            # 如果 role_scorecard 为空，尝试从 by_rule 转换（兼容旧格式）
            if not scorecard and "by_rule" in res:
                print("[DEBUG] 检测到旧格式 by_rule，正在转换为 role_scorecard...")
                for rule in res.get("by_rule", []):
                    # 提取角色名
                    role_name = rule.get("target_role", rule.get("role", "未知"))
                    # 提取状态
                    status = rule.get("status", "deferred")
                    # 提取 verdict 作为 core_view
                    core_view = rule.get("verdict", "")[:30]
                    if not core_view:
                        core_view = rule.get("brief_reason", "观点待补充")[:30]
            ...
            if missing_roles:
                ...
                # 重新分配贡献度（将缺失角色的贡献度均分给已有角色，或保持总和100%）
                existing_roles_list = [r for r in scorecard if r["role"] not in missing_roles]
                if existing_roles_list and len(missing_roles) > 0:
                    # 从已有角色中匀出贡献度给缺失角色
                    total_existing_contrib = sum(r.get("contribution_percent", 0) for r in existing_roles_list)
                    if total_existing_contrib > 0:
                        # 每个已有角色减少 5%，凑给缺失角色
                        for r in existing_roles_list:
                            r["contribution_percent"] = max(0, r.get("contribution_percent", 0) - 2)
                        # 给缺失角色分配贡献度
                        for r in scorecard:
                            if r["role"] in missing_roles:
                                r["contribution_percent"] = int(100 / len(scorecard)) if scorecard else 0
                    else:
                        # 如果没有已有贡献度，平均分配
                        per_role = int(100 / len(scorecard)) if scorecard else 0
                        for r in scorecard:
                            r["contribution_percent"] = per_role

            # ================================================================
            # 关键修复3：确保 final_verdict 和 rejected_items 有默认值
            # ================================================================
            if not res.get("final_verdict"):
                # 尝试从 by_rule 中提取 summary
                if "by_rule" in res and res["by_rule"]:
                    summaries = [r.get("verdict", "") for r in res["by_rule"] if r.get("verdict")]
                    if summaries:
                        res["final_verdict"] = "综合各角色观点，最终裁决如下：" + "；".join(summaries[:3])
                    else:
    ...
    def _empty_judge_result(self) -> Dict:
        ...
        for role in CORE_DEBATERS:
            scorecard.append({
                ...
                "alignment": 5,
                "activation": 5,
                "contribution_percent": int(100 / len(CORE_DEBATERS)) if role == CORE_DEBATERS[0] else 0,
                "status": "deferred",
                "brief_reason": "裁决异常"
            })
        # 确保贡献度总和为100
        if scorecard:
            scorecard[0]["contribution_percent"] = 100 - sum(r["contribution_percent"] for r in scorecard[1:])
        return {
            "role_scorecard": scorecard,
            "final_verdict": "裁决生成失败，请查看原始辩论记录",
            "rejected_items": []
        }
