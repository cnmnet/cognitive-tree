...
{final}"""
    ...
    def _debate_engine_reasoning(self, user_input: str, debate_mode: str, label: str, max_rounds: int = 2):
        ...
        try:
            ...
            try:
                ...
                print("[DEBUG] _render_result_to_panel 调用成功")
            except Exception as e:
            ...
            self.root.after(0, lambda: self._log(f"✅ {label} 完成！完整报告已生成 👆 请查看上方的「辩论报告」面板", "success"))
            self.root.after(0, lambda: self._log("=" * 60, "system"))
    ...
    def _render_result_to_panel(self, question: str, role_blocks: List[Dict],
        ...
        total_roles = len(role_blocks)
        self.result_area.insert(tk.END, "🎉 辩论完成！\n", "result_banner")
        self.result_area.insert(tk.END, f"  共 {total_rounds} 轮 · {total_roles} 位角色参与\n", "result_banner_sub")
        self.result_area.insert(tk.END, "─" * 60 + "\n", "result_divider")

        # 配置横幅标签（在方法内配置，确保存在）
        self.result_area.tag_config("result_banner",
                                    font=("微软雅黑", 18, "bold"),
                                    foreground="#1A237E",
                                    spacing1=10,
                                    spacing3=4)
        self.result_area.tag_config("result_banner_sub",
                                    font=("微软雅黑", 11),
                                    foreground="#5c6b7e",
                                    spacing1=0,
                                    spacing3=10)
        ...
        # ================================================================
        self.result_area.config(state=tk.DISABLED)
        self.result_area.see(1.0)  # 滚动到顶部

        # 背景色闪烁提示（吸引用户注意）
        original_bg = self.result_area.cget("bg")
        self.result_area.config(bg="#f0f8ff")  # 淡蓝色高亮
        self.root.after(1500, lambda: self.result_area.config(bg=original_bg))
...
"""
            ...
            if isinstance(raw_response, str):
                ...
                try:
                    ...
                    if res:
                        ...
                        return self._generate_fallback_kpi(CORE_DEBATERS, atomic)
            else:
    ...
    def _generate_fallback_kpi(self, core_debaters: List[str], atomic: Dict) -> Dict[str, Any]:
        """生成完整的降级KPI数据（7人×7项）"""
        scorecard = []
        for role in core_debaters:
            rv = atomic["role_contributions"].get(role)
            ...
            scorecard.append({
                ...
                "alignment": 5,
                "activation": 5,
                "contribution_percent": round(100 / len(core_debaters), 1),
                "status": "deferred",
                "brief_reason": "数据不足，系统自动补全"
            })
        # 修正贡献度总和为100%
        total = sum(r["contribution_percent"] for r in scorecard)
    ...
    def _parse_judge_from_text(self, text: str, core_debaters: List[str]) -> Dict[str, Any]:
        ...
        status_map = {
            "采纳": "adopted", "附条件采纳": "conditional", "暂缓": "deferred", "驳回": "rejected",
            "adopted": "adopted", "conditional": "conditional", "deferred": "deferred", "rejected": "rejected"
        }
